...
function git(args, cwd) {
    return new Promise((resolve) => {
        try {
            (0, child_process_1.execFile)("git", args, { cwd, encoding: "utf8", maxBuffer: 10 * 1024 * 1024 }, (err, stdout) => {
                if (err) {
                    resolve(null);
                    return;
                }
...
async function getGitContext(cwd) {
    ...
    const [branchRaw, statusRaw, logRaw, diffStatRaw] = await Promise.all([
        ...
        git(["log", "-n", "10", "--pretty=format:%H%x1f%s%x1f%ad", "--date=short"], dir),
        git(["diff", "--stat"], dir),
    ]);
    const context = { isRepo: true };
    if (branchRaw) {
