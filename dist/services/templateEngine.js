...
exports.checklistToTemplateData = checklistToTemplateData;
exports.worklogToTemplateData = worklogToTemplateData;
...
function templatesDir() {
    const candidates = [
        ...
        path.join(__dirname, "..", "templates"),
        path.join((0, config_1.configDir)(), "templates"),
...
function loadTemplate(kind, name) {
    ...
    throw new Error(`templateEngine: could not load ${kind} template (tried: ${tried.join(", ")})`);
}
/* ------------------------------------------------------------------ */
...
function numberedList(items, emptyText) {
    const clean = (items || []).map((i) => String(i).trim()).filter(Boolean);
    if (clean.length === 0)
        return `_${emptyText}_`;
    return clean.map((i, idx) => `${idx + 1}. ${i}`).join("\n");
}
function checklistTaskRow(t) {
    return `| ${escapeCell(t.status)} | ${escapeCell(t.priority)} | ${escapeCell(t.task)} | ${escapeCell(t.notes || "")} |`;
}
/** Convert a Checklist into the placeholder map the template expects. */
function checklistToTemplateData(c) {
    const header = "| STATUS | PRIORITY | TASK | NOTES |";
    const divider = "| --- | --- | --- | --- |";
    const rows = c.tasks && c.tasks.length > 0
...
function worklogCompletionRow(i) {
    return `| ${escapeCell(i.task)} | ${escapeCell(i.status)} | ${escapeCell(i.result || "")} |`;
}
function summaryFromWorklog(w) {
    if (w.summary && w.summary.trim())
        return w.summary.trim();
...
function timeGrid(w) {
    const t = w.time || {};
    const fmt = (n) => n === undefined || n === null || Number.isNaN(n) ? "—" : `${n.toFixed(1)} h`;
...
function worklogToTemplateData(w) {
    const header = "| PLANNED TASK | STATUS | RESULT |";
    const divider = "| --- | --- | --- |";
    const rows = w.checklistItems && w.checklistItems.length > 0
