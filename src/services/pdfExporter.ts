...
function makeCtx(doc: Doc, footer: string): Ctx {
...
function hairline(ctx: Ctx, x: number, y: number, w: number): void {
  ctx.doc
    .save()
    .lineWidth(0.75)
...
function parseRuns(input: string): Run[] {
  ...
  if (runs.length === 0) runs.push({ text: "", code: false, bold: false });
  return runs;
}
...
function drawRichText(
  ...
  // Tokenize into words tagged with code/bold flags; keep spaces.
  interface Tok {
    word: string;
    code: boolean;
    bold: boolean;
  ...
  for (const r of runs) {
    ...
    for (const w of words) toks.push({ word: w, code: r.code, bold: r.bold });
  }

  const fontFor = (t: Tok): string =>
    t.code ? FONT.mono : t.bold ? FONT.bold : baseFont;
 ...
 */
function completionCaption(ctx: Ctx, ref: string): void {
  const { doc } = ctx;
  const size = 9;
  const y = doc.y;
  const midY = y + 5;
...
function textLines(
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const h = Math.max(
      measureRichText(ctx, text || " ", textW, size),
      size + 3
    );
...
function blankLines(ctx: Ctx, count: number, rowH = 16): void {
  const { doc } = ctx;
...
interface TableRow {
  /** Draw a single cell; returns nothing, drawing bounded within box. */
  render: (
    ctx: Ctx,
    col: number,
    x: number,
  ...
  /** Measure the natural height of a cell (content only, no padding). */
  measure: (ctx: Ctx, col: number, w: number) => number;
}
...
function drawTable(ctx: Ctx, cols: Column[], rows: TableRow[]): void {
  ...
  const widths = cols.map((c, i) =>
    i === cols.length - 1 ? ctx.contentW - fixed : c.width
  );
...
function statRow(ctx: Ctx, stats: Stat[]): void {
  const { doc } = ctx;
...
function timeGrid(ctx: Ctx, cells: [string, string][]): void {
  const { doc } = ctx;
  const colW = ctx.contentW / 2;
  const cellH = 38;
...
function overallStatusRow(
  ctx: Ctx,
  selected: ScheduleStatus | null
): void {
  const { doc } = ctx;
  ensureSpace(ctx, 24);
  const y = doc.y + 4;
  ...
  const opts: { label: ScheduleStatus; dot: string }[] = [
...
function finalize(doc: Doc, outPath: string): Promise<string> {
...
export async function renderChecklistPDF(
  data: Checklist,
  outPath: string,
  opts: { blank?: boolean; footerNote?: string } = {}
): Promise<string> {
  ...
  const tasks: (ChecklistTask | null)[] = blank
  ...
  const rows: TableRow[] = tasks.map((t) => ({
    ...
    render: (c, col, x, y, w) => {
      ...
      if (col === 0) {
        ...
        if (t && t.notes) drawRichText(c, t.notes, x, y + 1, w, { size: 8.5 });
      }
    },
  }));
...
function fmtHours(v?: number): string {
  ...
  return `${(v as number).toFixed(1)} h`;
}

export async function renderWorklogPDF(
  data: Worklog,
  outPath: string,
  opts: { blank?: boolean; footerNote?: string } = {}
): Promise<string> {
  ...
  if (!blank) completionCaption(ctx, data.checklistRef || "");
  ...
  const items: (WorklogChecklistItem | null)[] = blank
  ...
  const cRows: TableRow[] = items.map((it) => ({
    ...
    render: (c, col, x, y, w) => {
      ...
      if (col === 0) {
        ...
        if (it) worklogStatusPill(c, it.status, x, y, w);
        else {
        ...
        if (it && it.result) drawRichText(c, it.result, x, y + 1, w, { size: 8.5 });
      }
    },
  }));
  ...
  if (blank) {
    ...
    const completed = data.checklistItems.filter(
      (i) => i.status === "Completed"
    ).length;
    const partial = data.checklistItems.filter(
      (i) => i.status === "Partial"
    ).length;
...
function twoColumnBlock(ctx: Ctx, data: Worklog, blank: boolean): void {
  const { doc } = ctx;
  const gutter = 20;
  ...
  const yLeft = renderColumn(
    leftX,
    "5. Blockers",
    "Anything slowing the work down.",
    data.blockers,
    false
  );
  const yRight = renderColumn(
    rightX,
    "6. Next Priorities",
