...
import { ScanFinding, FindingPriority } from "../types";
...
function walk(root: string): FileInfo[] {
  const out: FileInfo[] = [];
...
function inSourceDir(rel: string): boolean {
 ...
 */
export async function scanRepository(cwd?: string): Promise<ScanFinding[]> {
  const root = cwd || process.cwd();
  const findings: ScanFinding[] = [];
  const counts: Record<string, number> = {};

  const add = (f: ScanFinding): void => {
    const c = counts[f.category] || 0;
    if (c >= MAX_PER_CATEGORY) return;
  ...
  for (const file of sourceFiles) {
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      if (line.length > 4000) continue; // skip minified/generated lines
  ...
  for (const file of sourceFiles) {
    if (looksLikeTest(file.rel)) continue;
    const base = path.basename(file.rel, file.ext);
    if (ENTRY_HINTS.test(base)) continue;
    if (!inSourceDir(file.rel)) continue;
